	...
	while ( my $row = $rs_data->next ) {
		...
		push(
			@data, {
				...
				"defaultGeoLat"        => defined( $row->default_geo_lat ) ? 0.0 + $row->default_geo_lat : undef,
				"defaultGeoLong"       => defined( $row->default_geo_long ) ? 0.0 + $row->default_geo_long : undef,
...
sub update {
	...
	my $values = {
		...
		default_geo_lat        => $params->{defaultGeoLat},
		default_geo_long       => $params->{defaultGeoLong},
	...
	if ($rs) {
		...
		push(
			@response, {
				...
				"defaultGeoLat"            => defined($rs->default_geo_lat) ? 0.0 + $rs->default_geo_lat : undef,
				"defaultGeoLong"           => defined($rs->default_geo_long) ? 0.0 + $rs->default_geo_long : undef,
...
sub create {
	...
	my $values = {
		...
		miss_lat               => $params->{missLat},
		miss_long              => $params->{missLong},
		default_geo_lat        => $params->{default_geo_lat},
		default_geo_long       => $params->{default_geo_long},
		multi_site_origin      => $params->{multiSiteOrigin},
		org_server_fqdn        => $params->{orgServerFqdn},
		origin_shield          => $params->{originShield},
		profile                => $params->{profileId},
	...
	if ($insert) {
		...
		push(
			@response, {
				...
				"defaultGeoLat"            => defined($insert->default_geo_lat) ? 0.0 + $insert->default_geo_lat : undef,
				"defaultGeoLong"           => defined($insert->default_geo_long) ? 0.0 + $insert->default_geo_long : undef,
...
sub is_deliveryservice_valid {
	...
	my $rules = {
		...
		checks => [
			...
			defaultGeoLat		=> [ \&is_valid_number_or_undef ],
			defaultGeoLong		=> [ \&is_valid_number_or_undef ],
